...
namespace Task2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Regex regex = new Regex(@"^(?("")(""[^""]+?""@)|(([0-9a-z]((\.(?!\.))|[-!#\$%&'\*\+/=\?\^`\{\}\|~\w])*)(?<=[0-9a-z])@))" +@"(?(\[)(\[(\d{1,3}\.){3}\d{1,3}\])|(([0-9a-z][-\w]*[0-9a-z]*\.)+[a-z0-9]{2,17}))$");
            ...
            Regex r = new Regex(@"\b(0[1-9]\.0\d\.19\d{2}|[12]\d\.0\d\.19\d{2}|3[01]\.0\d\.19\d{2}|0[1-9]\.0\d\.200\d|[12]\d\.0\d\.200\d|3[01]\.0\d\.200\d|0[1-9]\.1[0-2]\.19\d{2}|[12]\d\.1[0-2]\.19\d{2}|3[01]\.1[0-2]\.19\d{2}|0[1-9]\.1[0-2]\.200\d|[12]\d\.1[0-2]\.200\d|3[01]\.1[0-2]\.200\d|0[1-9]\.1[0-2]\.20\d{2}|[12]\d\.1[0-2]\.20\d{2}|3[01]\.1[0-2]\.20\d{2}|0[1-9]\.0\d\.20\d{2}|[12]\d\.0\d\.20\d{2}|3[01]\.0\d\.20\d{2})\b");
